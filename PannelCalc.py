...
colors = {'코르텐블랙': '프린트', '코르텐밤색': '프린트', '헤어진회': '프린트', '헤어연회': '프린트', '헤어골드': '프린트', '빅징크': '프린트', '라인메탈실버': '프린트',
          '헤어은회': '프린트', '골드메탈': '프린트', '콘크리트': '프린트', '메탈스톤녹색': '프린트', '메탈스톤블루': '프린트', '메탈스톤실버': '프린트', '불소골드': '프린트',
          '백색엠보': '단색', '우드엠보': '프린트', '진우드엠보': '프린트', '밤색슁글': '프린트', '스패니쉬': '프린트', '다크스패니쉬': '프린트', '밝은스패니쉬': '프린트',
          '멀티쿠퍼': '프린트', '우드': '프린트', '링클진회': '스톤', '링클브라운': '스톤', '링클검정': '스톤', '링클동색': '스톤', '빅스톤다크브라운': '빅스톤',
          '빅스톤검정': '빅스톤', '빅스톤자주': '빅스톤', '빅스톤다크블루': '빅스톤', '블루매트': '매트', '다크블루매트': '매트', '검정매트': '매트', '브라운매트': '매트',
          '진밤매트': '매트', '자주매트': '매트', '아이보리': '단색', '포스맥원판': '단색', '갈바': '단색', '은회': '단색', '진청': '단색', '밤색': '단색',
          '자주': '단색', '녹색': '단색', '단색진회': '단색', '오렌지': '단색', '오션블루': '단색', '크림화이트': '단색', '베이지': '단색', '아연': '단색',
          '티타늄실버': '단색', '헤어검정': '프린트'}
colors_width = {'코르텐블랙': '1219,607', '코르텐밤색': '1219,607', '헤어진회': '1219,607,518', '헤어연회': '1219,607',
                '헤어골드': '1219,607', '빅징크': '1219', '라인메탈실버': '1150', '헤어은회': '1150', '골드메탈': '1150', '콘크리트': '1150',
...
EtcPrice = {'징크판넬': '850', '지붕': '600', '메탈판넬': '850', '사이딩': '850', '벽체': '600', '외벽판넬': '600', 'V250': '0',
            'V115': '0', 'OL250': '0', 'OL330': '0', 'V250(PF)': '700', 'V115(PF)': '900', 'OL250(PF)': 'OL330(PF)',
...
def coilcolor2(color):  # 강판
    ...
    if color_type == '단색':
        ...
        colorprice = 2000
        # if "링클" in color : colorprice = 1000
        if color == '불소골드': colorprice = 3600
    elif color_type == '스톤':
        colorprice = 1800
...
def coilpricecalc(name, color, top, bot, board, rate):  # 품목명,색상,상판폭,하판폭,보드단가,판매요율
    ...
    # except: print('coilpricecalc 문제 발생')
    # width = CoilWidth['상판폭'][name] # 폭 검색
    # search_width = CoilThick['폭'] == str(width)
    # search_top = CoilThick['두께'] == str(top)
    # gravity = CoilThick[search_width & search_top]['단중'].values #두께 & 폭 열에 맞는 값을 찾고 단중의 값 뽑아냄
    # # print(f"상판 폭 : {width} 단중 : {gravity}")
    # rate = clientrate(name,rate) # 판매 요율
    # price1 = float(gravity) * (coilprice) # 상판 가격
    # price2 = coilcolor2(color) # 색상 가격
    # price3 = etcprice(name) #기타 비용
    # price = tenup(price1 * rate) + float(price2) + float(price3) 
    # return int(price),price1,price2,price3
    # print(f"판매 요율 : {rate} /상판가격 :{price1} / 색상가격 : {price2} / 기타비용 :{price3} / 총 가격 : {price}")
...
def renamebuja(names):
    r_name = names
    rename = {'스크류볼트': '볼트', '물홈통': '이형후레싱', '받침대': 'PVC까치발', '코너커버': '코너카바', '유도모음집': '유도모임통', '반도': '선홈통반도',
              '물받이': '돌출물받이', '엘바': '아연엘바'}
    if names in rename:
        name = rename[names]
        return name
